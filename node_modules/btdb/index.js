...
var db = new Database('public/dat/db/car.db');
...
var stm=db.prepare('select *,rowid from tmp ');
var row=stm.all();
return row;
};
exports.skuTmp=function(sku){
var stm=db.prepare('select * from tmp where sku=?');
try{var row=stm.get(usr,sku);
}catch(err){console.log(err);}
return row;
};
exports.usrTmp=function(usr,sku){
var stm=db.prepare('select * from tmp where usr=? and sku=?');
try{var row=stm.all(usr,sku);
}catch(err){console.log(err);}
return row;
};
// === tmp run ===
